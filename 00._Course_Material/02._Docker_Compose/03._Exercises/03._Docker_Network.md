...
```bash
docker run --name my_mysql --network my-network -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=mydb -e MYSQL_USER=user -e MYSQL_PASSWORD=secret mysql:8.0
```
...
Use the spring application from the previous exercise, and build the image from where the `Dockerfile` is located:
...
We want to stop the container before we proceed but `docker stop` doesn't free up the port, so we need to remove the container completely:

```bash
docker rm -f my_web_app
```
