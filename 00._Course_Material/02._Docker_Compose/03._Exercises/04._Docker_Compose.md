...
In this exercise we will create a `compose.yaml` file to define a database, and a web application that connects to it.

## YAML syntax
YAML is a human-readable data serialization format. It is commonly used for configuration files and data exchange between languages with different data structures. In a `compose.yaml` file, you define your application's services, networks, and volumes using YAML syntax.

YAML uses indentation to represent the structure of the data. Each level of indentation represents a new level in the hierarchy. For example, in the `compose.yaml` file, the services are defined at the top level, and each service has its own set of properties indented beneath it.
...
Create a new file named `compose.yaml` in the root of your project directory.
...
To start the container defined in the `compose.yaml` file, you can use the following command:
```bash
docker compose up -d
```

This command will start the services (here only the MySQL db) defined in the `compose.yaml` file in **detached mode**.
...
SHOW DATABASES;
```
You should see the `mydb` database that we created using the environment variables in the `compose.yaml` file.
...
To stop the containers defined in the `compose.yaml` file, you can use the following command:

```bash
docker compose down
```

This command will stop and remove all the services defined in the `compose.yaml` file.
...
Fortunately, adding volume support in the `compose.yaml` file is straightforward. Replace the `db` service definition with the following:
...
## Step 9: Cleaning up
To stop and remove the containers defined in the `compose.yaml` file, you can use the following command:
