...
Create a new file called `compose.prod.yaml` in the same directory as your `compose.yaml` file (**root**).

In the `compose.prod.yaml` file, copy over the configuration from `compose.yaml`.
...
Create a new file called `nginx.conf` in the same directory as your `compose.prod.yaml` file. This file will contain the configuration for the reverse proxy.
...
In `compose.prod.yaml` file, add the nginx service:
...
In the `compose.prod.yaml` file, add a health check to the app service:
...
Start the services but remember to rebuild (`--build`) the app service, since we added a new dependency:
