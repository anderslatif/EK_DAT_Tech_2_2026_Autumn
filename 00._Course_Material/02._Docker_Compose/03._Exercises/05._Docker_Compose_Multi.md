...
We will reuse the `compose.yaml` file from the previous exercise and add a new service for the Spring Boot application.
...
First stop the two containers by running (`-v` removes the volumes defined in the `compose.yaml` file as well):

```bash
docker compose down -v
```

To make sure that the database is ready before the Spring Boot application starts, we can add a health check to the MySQL service in the `compose.yaml` file.
...
1. Run the containers again.
2. Check if both containers are running.
3. Check the logs of the Spring Boot application and verify that it has successfully connected to the MySQL database.


## Step 4: Tear down

Run the following command to stop and remove the containers, and volumes defined in the `compose.yaml` file:
