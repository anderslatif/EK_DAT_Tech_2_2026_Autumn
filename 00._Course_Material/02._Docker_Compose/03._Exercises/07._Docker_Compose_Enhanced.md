...
Improve the `compose.yaml` setup by adding restart policies to the services, adding Spring profiles support and we will look at port exposure.
...
To activate a specific profile, we can set the `SPRING_PROFILES_ACTIVE` environment variable in our `compose.yaml` file.
...
Then we could reference it inside the `compose.yaml` file:
...
In case the application fails we want the services to restart automatically. We can achieve this by adding a `restart` policy section to our `compose.yaml` file.
