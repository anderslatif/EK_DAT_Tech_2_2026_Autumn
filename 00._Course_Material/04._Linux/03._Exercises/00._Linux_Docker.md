# Running a Linux Container with Docker Compose
...
2. Download one of your GitHub repositories using `wget` (remember to replace `<username>` and `<repo>` with your actual GitHub username and repository name):
    ```bash
...
1. When you are done, you can `exit`the container and then stop and remove it with:
	```bash
	docker compose down
	```
...
**Tip:** You can edit the `compose.yml` file to try other Linux images (like `alpine`, etc.) or add more services.
