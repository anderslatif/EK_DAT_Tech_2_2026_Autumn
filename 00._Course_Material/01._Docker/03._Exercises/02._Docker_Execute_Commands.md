...
Detached mode means that the container will run in the background, allowing you to continue using your terminal.
...
This command will start a new nginx container in detached mode (`-d`), mapping port `8090` on the host to port `80` in the container, and give the custom name `my-nginx` to the container.
