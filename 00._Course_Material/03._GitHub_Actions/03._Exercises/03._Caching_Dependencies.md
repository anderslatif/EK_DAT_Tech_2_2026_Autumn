...
```bash
git add -A
git commit -m "Added caching to Maven build GitHub Action"
git push
```
...
5. To see the caching in action, click on the **Run workflow** button in the top right corner of the Actions tab after you've clicked on "Java test with caching". This is available because of the `workflow_dispatch` event in the workflow file.

Observe the action run again. The first run after the change will take longer as it rebuilds the cache, but subsequent runs should be faster.
