...
- `http://localhost:8090/api/hello` - returns "Hello, World - updated!"
...
This YAML file defines a GitHub Action that triggers on every push to the repository. It checks out the code and then runs a script that prints "Hello, World!" to the console in the Ubuntu runner.
...
```bash
git add -A
git commit -m "Added Hello World GitHub Action"
git push
```
