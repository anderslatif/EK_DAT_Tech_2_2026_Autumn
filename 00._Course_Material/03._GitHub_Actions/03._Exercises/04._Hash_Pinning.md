...
Go to the GitHub repository of the action you are using. For example, for `actions/checkout`, go to [actions/checkout](https://github.com/actions/checkout). Find the release page. By clicking the short hash, you will reach the commit page and the full SHA can be copied from the URL.

Find the commit you want to use and copy its hash (for this exercise we will use the latest commit hash). On Jul 20th, 2026, the latest commit hash for `v7.0.1` for `actions/checkout` is `3d3c42e5aac5ba805825da76410c181273ba90b1`. You will need to update it with the newest verified commit hash that you can find.
...
Repeat the process for any other actions you are using in your workflow. For example, for `actions/setup-java` in the same file. 
