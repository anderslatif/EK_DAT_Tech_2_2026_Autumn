...
1. Create a new user on the VM for your fellow student (replace `studentname` with their username):
	```bash
...
1. Create the `.ssh` directory in the new user's home directory and set the correct permissions:
...
2. Still on your VM, fetch your fellow student's public key from GitHub (replace `githubusername` and `studentname`):
	```bash
	curl https://github.com/githubusername.keys | sudo tee -a /home/studentname/.ssh/authorized_keys
